...
HandleReadEvent(struct thread_context *ctx, int sockid, 
				uint32_t rpc_ind, struct server_vars *sv)
{
	//struct mtcp_epoll_event ev;
	...
	rd = mtcp_rpc_read(ctx->mctx, sockid, rpc_ind, &req);
	...
	memcpy(sv->request + sv->recv_len, 
			(char *)req.buff, MIN(rd, HTTP_HEADER_LEN - sv->recv_len));
	sv->recv_len += rd;
	//sv->request[rd] = '\0';
...
/*----------------------------------------------------------------------------*/
int 
CreateRPCSocket(struct thread_context *ctx)
{
	int listener;
	struct mtcp_epoll_event ev;
	struct sockaddr_in sock_saddr;

	/* create socket, set it as nonblocking, and bind it to port 80 */
	/* bind to port 80 */
	sock_saddr.sin_family = AF_INET;
	// saddr.sin_addr.s_addr = INADDR_ANY;
	sock_saddr.sin_addr.s_addr = inet_addr(server_ip);
	sock_saddr.sin_port = htons(80);
	listener = mtcp_rpc_socket(ctx->mctx, AF_INET, 0,
							   (struct sockaddr *)&sock_saddr, sizeof(struct sockaddr_in));
	if (listener < 0) {
		TRACE_ERROR("Failed to create RPC socket!\n");
		return -1;
	}
