...
CreateRPCSocket(struct thread_context *ctx){
	...
	sock_daddr.sin_addr.s_addr = daddr;
	sock_daddr.sin_port = dport;

	int ret = mtcp_find_saddr(ctx->mctx, &sock_saddr, &sock_daddr);
	sock_saddr.sin_family = AF_INET;
	
	if (ret < 0){
		TRACE_ERROR("Failed to find socket source address!\n");
		exit(EXIT_FAILURE);	
	}
...
RunWgetMain(void *arg)
{
	...
	// Have to add a UDP header for this to work
	...
	int client = CreateRPCSocket(ctx);
	if (client < 0) {
		TRACE_ERROR("Failed to create RPC socket!\n");
		exit(EXIT_FAILURE);	
	}

	printf("Successfully created RPC socket %d\n", client);
...
//			"Connection: Keep-Alive\r\n\r\n", 
	...
	mcfg.sndbuf_size = next_power_of_two(total_len + 1) * 2;
	mtcp_setconf(&mcfg);
