...
#define REPORT_INTERVAL 0.5		// 0.5s
/*----------------------------------------------------------------------------*/
...
{
	if (mode == RECV_MODE || mode == 0) {
		ERROR("(client initiates)   usage: ./client send [ip] [port]");
	}
	if (mode == WAIT_MODE || mode == 0) {
		ERROR("(server initiates)   usage: ./client wait");
	}
}
/*----------------------------------------------------------------------------*/
int
main(int argc, char **argv) 
{
	...
	if (mode == WAIT_MODE) {
		...
		while (1) { // loop until connected to a client, break when we can receive
		...
		for (int i = 0; i < events_ready; i++) {
			...
			if (events[i].events & MTCP_EPOLLIN) {
				...
				if (read <= 0) {
					...
					// Measure throughput every 0.5s
					gettimeofday(&now, NULL);
					elapsed_time = (now.tv_sec - last_report.tv_sec) + (now.tv_usec - last_report.tv_usec) / 1000000.0;
					if (elapsed_time >= REPORT_INTERVAL) {
						double throughput = (bytes_rcvd * 8.0 / 1000000.0) / elapsed_time; // Mbps
						DEBUG("Time elapsed: %.2f sec, bytes received: %d, Throughput: %.3f Mbps",
							elapsed_time, bytes_rcvd, throughput);
					...
					// Check for termination byte (\x96)
					if (rcvbuf[read - 1] == '\x96') {
						DEBUG("Got FIN from sender.");
						mtcp_write(ctx->mctx, sockfd, "OK", 2);
						DEBUG("Sent acknowledgment to sender");
						goto stop_timer;
					}
				}
			}
...
stop_timer:
	...
	printf("Throughput: %.3fMbit/sec\n", ((total_rcvd * 8.0 / 1000000.0) / elapsed_time));
