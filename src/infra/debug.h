...
#include <stdio.h>
#include <assert.h>
#include <netinet/ip.h>	/* struct iphdr, for the dumpers below.
				 * mTCP gets it via tcp_in.h, which this
				 * layer must not include. */
#include "infra.h"
...
#define TRACE_TEMP(f, m...) { \
	fprintf(stderr, "[CPU %d][%10s:%4d] " f, core->ctx->cpu, 		\
...
#define CTRACE_ERROR(f, m...) { \
	fprintf(stderr, "[CPU %d][%10s:%4d] " f, core->ctx->cpu, __FUNCTION__, __LINE__, ##m);	\
	}
...
#define TRACE_DBG(f, m...) {\
	thread_printf(core, core->log_fp, "[%10s:%4d] " \
...
#define TRACE_FUNC(n, f, m...) {                                         \
	thread_printf(core, core->log_fp, "[%6s: %10s:%4d] " \
