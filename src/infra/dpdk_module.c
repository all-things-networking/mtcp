...
/*----------------------------------------------------------------------------*/
void
dpdk_init_handle(struct thread_ctx *ctxt)
{
...
/*----------------------------------------------------------------------------*/
int
dpdk_link_devices(struct thread_ctx *ctxt)
{
...
/*----------------------------------------------------------------------------*/
void
dpdk_release_pkt(struct thread_ctx *ctxt, int ifidx, unsigned char *pkt_data, int len)
{
	/*
...
/*----------------------------------------------------------------------------*/
int
dpdk_send_pkts(struct thread_ctx *ctxt, int ifidx)
{
	struct dpdk_private_context *dpc;
#ifdef NETSTAT
	core_ctx_t core;
...
#ifdef NETSTAT
		core->nstat.tx_packets[ifidx] += cnt;
#ifdef ENABLE_STATS_IOCTL
		/* only pass stats after >= 1 sec interval */
		if (abs(core->cur_ts - dpc->cur_ts) >= 1000 &&
		    likely(dpc->fd >= 0)) {
			...
			ss.tx_pkts = core->nstat.tx_packets[ifidx];
			ss.tx_bytes = core->nstat.tx_bytes[ifidx];
			ss.rx_pkts = core->nstat.rx_packets[ifidx];
			ss.rx_bytes = core->nstat.rx_bytes[ifidx];
			ss.qid = ctxt->cpu;
			ss.dev = portid;
			/* pass the info now */
			if (ioctl(dpc->fd, SEND_STATS, &ss) == -1)
				TRACE_ERROR("Can't update iface stats!\n");
			dpc->cur_ts = core->cur_ts;
			if (ctxt->cpu == 0)
...
/*----------------------------------------------------------------------------*/
uint8_t *
dpdk_get_wptr(struct thread_ctx *ctxt, int ifidx, uint16_t pktsize)
{
	struct dpdk_private_context *dpc;
#ifdef NETSTAT
	core_ctx_t core;
...
#ifdef NETSTAT
	core->nstat.tx_bytes[ifidx] += pktsize + ETHER_OVR;
#endif
...
/*----------------------------------------------------------------------------*/
int32_t
dpdk_recv_pkts(struct thread_ctx *ctxt, int ifidx)
{
	struct dpdk_private_context *dpc;
	int ret;
...
/*----------------------------------------------------------------------------*/
uint8_t *
dpdk_get_rptr(struct thread_ctx *ctxt, int ifidx, int index, uint16_t *len)
{
	struct dpdk_private_context *dpc;
	struct rte_mbuf *m;
	uint8_t *pktbuf;
...
/*----------------------------------------------------------------------------*/
int32_t
dpdk_select(struct thread_ctx *ctxt)
{
#ifdef RX_IDLE_ENABLE
	struct dpdk_private_context *dpc;
...
/*----------------------------------------------------------------------------*/
void
dpdk_destroy_handle(struct thread_ctx *ctxt)
{
	struct dpdk_private_context *dpc;
	int i;
...
/*----------------------------------------------------------------------------*/
int32_t
dpdk_dev_ioctl(struct thread_ctx *ctx, int nif, int cmd, void *argp)
{
	struct dpdk_private_context *dpc;
	struct rte_mbuf *m;
	int len_of_mbuf;
	struct iphdr *iph;
	struct l4_csum_req *req = NULL;
	uint16_t *l4_csum;
	...
	/* mTCP passes `struct iphdr *` for every command and casts to
	 * `struct tcphdr` inside. The two commands that touch the L4 header take
	 * a request instead, carrying the two numbers that used to be read out
	 * of a header this layer must not know the shape of. */
	if (cmd == PKT_TX_L4_CSUM || cmd == PKT_TX_L3L4_CSUM) {
		req = (struct l4_csum_req *)argp;
		iph = req->iph;
	} else {
	...
	case PKT_TX_L4_CSUM:
		...
		l4_csum = (uint16_t *)((uint8_t *)iph + (iph->ihl<<2) + req->l4_csum_offset);
		*l4_csum = rte_ipv4_phdr_cksum((struct rte_ipv4_hdr *)iph, m->ol_flags);
		break;
	/* mTCP has a PKT_RX_L4_LROSEG case here that walks a coalesced segment
	 * chain and reads the L4 header to find the payload. LRO is off in the
	 * donor's own build (its Makefile sets LRO=0), so nothing that has been
	 * measured uses it, and it is the second place the PMD glue would need
	 * to know the shape of an L4 header. Not carried. */
	case PKT_TX_L3L4_CSUM:
		if ((dev_info[nif].tx_offload_capa & RTE_ETH_TX_OFFLOAD_IPV4_CKSUM) == 0)
			goto dev_ioctl_err;
		if ((dev_info[nif].tx_offload_capa & RTE_ETH_TX_OFFLOAD_TCP_CKSUM) == 0)
			goto dev_ioctl_err;
