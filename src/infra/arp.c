...
/*----------------------------------------------------------------------------*/
void 
DumpARPPacket(core_ctx_t core, struct arphdr *arph);
/*----------------------------------------------------------------------------*/
int 
InitARPTable()
{
...
/*----------------------------------------------------------------------------*/
static int 
ARPOutput(struct core_ctx *core, int nif, int opcode,
		uint32_t dst_ip, unsigned char *dst_haddr, unsigned char *target_haddr)
{
	...
	/* Allocate a buffer */
	struct arphdr *arph = (struct arphdr *)EthernetOutput(core, 
...
RequestARP(core_ctx_t core, uint32_t ip, int nif, uint32_t cur_ts)
{
	...
	ARPOutput(core, nif, arp_op_request, ip, haddr, taddr);
}
/*----------------------------------------------------------------------------*/
static int 
ProcessARPRequest(core_ctx_t core, 
		struct arphdr *arph, int nif, uint32_t cur_ts)
{
	...
	/* send arp reply */
	ARPOutput(core, nif, arp_op_reply, arph->ar_sip, arph->ar_sha, NULL);

	return 0;
}
/*----------------------------------------------------------------------------*/
static int 
ProcessARPReply(core_ctx_t core, struct arphdr *arph, uint32_t cur_ts)
{
	unsigned char *temp;
	struct arp_queue_entry *ent;
		                  ...
		                  const int ifidx, unsigned char *pkt_data, int len)
{
	...
	switch (ntohs(arph->ar_op)) {
		case arp_op_request:
			...
			ProcessARPRequest(core, arph, nif, cur_ts);
			break;

		case arp_op_reply:
			ProcessARPReply(core, arph, cur_ts);
			break;
...
ARPTimer(core_ctx_t core, uint32_t cur_ts)
{
	...
	TAILQ_FOREACH_SAFE(ent, &g_arpm.list, arp_link, ent_tmp) {
		if (SEQ_GT(cur_ts, ent->ts_out + SEC_TO_TS(ARP_TIMEOUT_SEC))) {
			struct in_addr ina;
			ina.s_addr = ent->ip;
			TRACE_INFO("[CPU%2d] ARP request for %s timed out.\n", 
				   core->ctx->cpu, inet_ntoa(ina));
			TAILQ_REMOVE(&g_arpm.list, ent, arp_link);
			free(ent);
		}
	}
...
DumpARPPacket(core_ctx_t core, struct arphdr *arph)
{
	...
	thread_printf(core, core->log_fp, "ARP header: \n");
	thread_printf(core, core->log_fp, "Hardware type: %d (len: %d), "
