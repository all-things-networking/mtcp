...
struct core_ctx *g_core[MAX_CPUS] = 	{NULL};
struct infra_config CONFIG = {
...
EnrollRouteTableEntry(char *optstr)
{
	...
	ifidx = -1;
	/* mTCP branches over four I/O modules here; only this one is carried */
...
ParseConfiguration(char *line)
{
    ...
    } else if (ProgConfigKey(p, q)) {
		/* a key this layer has no business knowing — a connection timeout,
		 * for instance. The donor keeps those two in its own config struct;
		 * here they belong to the program, which is the only thing that
		 * knows what a connection is (rule 4, DESIGN.md §7.2). */
	} else {
...
LoadConfiguration(const char *fname)
{
	...
	TRACE_CONFIG("----------------------------------------------------------"
			"-----------------------\n");
	TRACE_CONFIG("Loading core configuration from : %s\n", fname);
