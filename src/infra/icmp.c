...
/*----------------------------------------------------------------------------*/
void 
DumpICMPPacket(core_ctx_t core, struct icmphdr *icmph, uint32_t saddr, uint32_t daddr);
/*----------------------------------------------------------------------------*/
static uint16_t
...
/*----------------------------------------------------------------------------*/
static int
ICMPOutput(struct core_ctx *core, uint32_t saddr, uint32_t daddr,
	   uint8_t icmp_type, uint8_t icmp_code, uint16_t icmp_id, uint16_t icmp_seq,
	   uint8_t *icmpd, uint16_t len)
{
	struct icmphdr *icmph;
	
	icmph = (struct icmphdr *)IPOutputStandalone(core, 
...
#if DBGMSG
	DumpICMPPacket(core, icmph, saddr, daddr);
#endif
	return 0;
}
/*----------------------------------------------------------------------------*/
void
RequestICMP(core_ctx_t core, uint32_t saddr, uint32_t daddr,
	    uint16_t icmp_id, uint16_t icmp_sequence,
	    uint8_t *icmpd, uint16_t len)
{
	/* send icmp request with given parameters */
	ICMPOutput(core, saddr, daddr, ICMP_ECHO, 0, ntohs(icmp_id), ntohs(icmp_sequence),
		   icmpd, len);
}
/*----------------------------------------------------------------------------*/
static int 
ProcessICMPECHORequest(core_ctx_t core, struct iphdr *iph, int len)
{
	int ret = 0;
	...
	if (ICMPChecksum((uint16_t *) icmph, len - (iph->ihl << 2)) )
		...
		ICMPOutput(core, iph->daddr, iph->saddr, ICMP_ECHOREPLY, 0, 
...
/*----------------------------------------------------------------------------*/
int 
ProcessICMPPacket(core_ctx_t core, struct iphdr *iph, int len)
{
	struct icmphdr *icmph = (struct icmphdr *) IP_NEXT_PTR(iph);
	int i;
	int to_me = FALSE;
	...
	switch (icmph->icmp_type) {
        case ICMP_ECHO:
		ProcessICMPECHORequest(core, iph, len);
		break;
...
DumpICMPPacket(core_ctx_t core, struct icmphdr *icmph, uint32_t saddr, uint32_t daddr)
{
	...
	thread_printf(core, core->log_fp, "ICMP header: \n");
	thread_printf(core, core->log_fp, "Type: %d, "
		      "Code: %d, ID: %d, Sequence: %d\n", 
		      icmph->icmp_type, icmph->icmp_code,
		      ntohs(ICMP_ECHO_GET_ID(icmph)), ntohs(ICMP_ECHO_GET_SEQ(icmph)));
	
	t = (uint8_t *)&saddr;
	thread_printf(core, core->log_fp, "Sender IP: %u.%u.%u.%u\n",
		      t[0], t[1], t[2], t[3]);
	
	t = (uint8_t *)&daddr;
	thread_printf(core, core->log_fp, "Target IP: %u.%u.%u.%u\n",
		      t[0], t[1], t[2], t[3]);
}
/*----------------------------------------------------------------------------*/
#undef IP_NEXT_PTR
