...
#include "infra.h"	/* first: MAX_DEVICES, before the kmod header guesses it */
#include "io_module.h"
...
SetNetEnv(char *dev_name_list, char *port_stat_list)
{
	...
	if (current_iomodule_func == &dpdk_module_func) {
		 ...
		 * core applications that also use an `external' getopt
...
{
	/* mTCP also checks whether the netmap device is openable; not carried.
