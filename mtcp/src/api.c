...
mtcp_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	MTP_PRINT("after MtpAcceptChain: accepted:%p\n", accepted);
	if (res == NULL) {
		return -1;
	}
	accepted = res->stream;
...
CopyToUser(mtcp_manager_t mtcp, tcp_stream *cur_stream, char *buf, int len)
{
	...
	memcpy(buf, rcvvar->rcvbuf->head, copylen);
	//TODO watch this (change it to cur_stream->mtp_rbm) if needed
...
/*----------------------------------------------------------------------------*/
int mtcp_rpc_send_resp(mctx_t mctx, int sockid, uint32_t rpc_ind, 
					   char* msg_hdr, size_t msg_hdr_len, char* buf, size_t len){
	mtcp_manager_t mtcp;
	socket_map_t socket;
	...
	tcp_stream* cur_stream = socket->rpcs[rpc_ind];
	
	int ret = MtpHomaSendRespChainPart1(mtcp, mtcp->cur_ts, msg_hdr,
							  msg_hdr_len, buf, len, cur_stream);
