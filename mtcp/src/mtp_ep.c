...
void mtp_syn_chain(mtcp_manager_t mtcp, uint32_t cur_ts,
	...
	uint32_t cur_ts, const int ifidx, const struct iphdr *iph, int ip_len)
{
    ...
    if (is_syn && !is_ack){
        ...
        mtp_syn_chain(mtcp, cur_ts, iph->saddr, tcph->source, seq, window, 
	...
	if (payloadlen > 0){
		mtp_data_chain(mtcp, cur_stream, cur_ts, seq, payload, payloadlen);
    } 
