...
void first_req_pkt_ep (mtcp_manager_t mtcp, uint32_t cur_ts,
							uint32_t ev_seq,
    ...
    if (scratch->complete) {
		...
		// printf("raise read event. sec: %ld, usec: %ld\n", cur_tv.tv_sec, cur_tv.tv_usec);
...
void next_req_pkt_ep (mtcp_manager_t mtcp, uint32_t cur_ts,
	...
	MTP_PRINT("RPC ind: %d, merged buffer length: %d\n", 
    ...
    if (scratch->complete) {
		MTP_PRINT("Read event in next_req_pkt_eq for RPC ind: %d\n", cur_stream->rpc_ind);
		// struct timeval cur_tv;
		// gettimeofday(&cur_tv, NULL);
		// printf("raise read event. sec: %ld, usec: %ld\n", cur_tv.tv_sec, cur_tv.tv_usec);
        RaiseReadEvent(mtcp, cur_stream);
...
void recv_resp_ep (mtcp_manager_t mtcp, uint32_t cur_ts,
    ...
    if(new_state) {
        ...
        if(ev_single_packet) {
			...
			MTP_PRINT("Read event in recv resp single packet for RPC ind: %d\n", cur_stream->rpc_ind);
			RaiseReadEvent(mtcp, cur_stream);
        ...
        if(scratch->complete) {
			...
			MTP_PRINT("Read event in recv resp multiple packet for RPC ind: %d\n", cur_stream->rpc_ind);
			RaiseReadEvent(mtcp, cur_stream);
        }
...
void print_ri_list(){
	MTP_PRINT("------- MTP_ri list:---------\n");
	for (int i = 0; i < MTP_HOMA_OVERCOMMITMENT; i++){
		MTP_PRINT("rpc_id: %d, newgrant: %d, priority: %d\n",
...
void print_rpc_info_1(char* name, rpc_info_1 *ri){
	MTP_PRINT("%s: peer_id: %d, rpc_id: %d, bytes_remaining: %d\n",
...
void sched_ep (mtcp_manager_t mtcp, uint32_t cur_ts,
    ...
    if (equal_sorted_list_1(&elem, &highest_prio)) {
        ...
        if (old_ind < 0 || MTP_all_rpcs[old_ind].peer_id != elem.peer_id){
			...
			MTP_PRINT("In this branch of sched_ep: old_ind: %d\n", old_ind);
            //remove previous highest priority
            rpc_info_2 old_prio_elem;
            rpc_info_1 old_elem = MTP_all_rpcs[old_ind];
            old_prio_elem.bytes_remaining = old_elem.bytes_remaining;
...
void choose_grant_ep(mtcp_manager_t mtcp, uint32_t cur_ts, scratchpad *scratch){
    ...
    for (int i = 0; i < 8; i++){
		...
		MTP_PRINT("RPCID: %d, increment: %d, available:%d, new_grant: %u, grant_elem.incoming: %d\n",
...
tcp_stream* MtpHomaSendReqChainPart1(mtcp_manager_t mtcp, uint32_t cur_ts, char* buf,
	...
	MTP_PRINT("Creating stream with srcip %u, sport %u, destip %u, destport %u, rpcid %u\n",
...
void MtpHomaNoHomaCtxChain (mtcp_manager_t mtcp, uint32_t cur_ts,
	...
	printf("tdiff reset grant state:%ld\n", tdiff);
	#endif
}
...
void MtpHomaRecvdReqChain (mtcp_manager_t mtcp, uint32_t cur_ts,
	...
	// printf("tdiff next_req_pkt_ep:%ld\n", tdiff);
	...
	printf("tdiff req_recv_chain:%ld\n", tdiff);
